<x-app-layout>
    <form method="post" action="{{ route('discussions.store') }}">
        ...
        <div>
            ...
            <select name="category_id" id="category" required>
                @foreach ($categories as $category)
                    <option @selected($category->id === $discussion->category_id) value='{{ $category->id }}'>{{ $category->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <x-form-input type='text' name='title' id='title' value="{{ old('title') ? old('title') : $discussion->title }}"/>
            <x-input-error name='title'></x-input-error>
        </div>
        <div>
            <x-form-label for='content'>Content</x-form-label>
            <x-text-area name='content' id='content'>{{old('content') ? old('content') : $discussion->content}}</x-text-area>
